# api/job.py
from fastapi import APIRouter, Depends, HTTPException, status
from sqlmodel import Session
from models import db_models
from core import db
from core.security import get_current_user
from core.logger import get_logger
from core.job import get_job_status, get_job_result

logger = get_logger(__name__)

router = APIRouter()

@router.get("/status/{job_id}", status_code=200)
def get_job_status_api(
    job_id: str,
    session: Session = Depends(db.get_session),
    current_user: db_models.UserProfile = Depends(get_current_user)
):
    logger.info(f"Get status for job_id={job_id} by user={current_user.username}")
    try:
        status = get_job_status(
            session=session,
            current_user=current_user,
            job_id=job_id)
        logger.info(f"Status for job_id={job_id}: {status}")
        return {"job_id": job_id, "status": status}
    except ValueError as e:
        logger.warning(f"Job not found or unauthorized for job_id={job_id}")
        raise HTTPException(status_code=404, detail=str(e))

@router.get("/result/{job_id}", status_code=200)
def get_job_result_api(
    job_id: str,
    session: Session = Depends(db.get_session),
    current_user: db_models.UserProfile = Depends(get_current_user)
):
    logger.info(f"Get result for job_id={job_id} by user={current_user.username}")
    try:
        result = get_job_result(
            session=session,
            current_user=current_user,
            job_id=job_id)
        logger.info(f"Result for job_id={job_id}: {result}")
        return {"job_id": job_id, "result": result}
    except ValueError as e:
        logger.warning(f"Job not found or unauthorized for job_id={job_id}")
        raise HTTPException(status_code=404, detail=str(e))